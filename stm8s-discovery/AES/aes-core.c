...
static const u8 __rcon[] = {
	0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

#define xtime2_sbox(x) (__xtime2_sbox[x])
static const u8 __xtime2_sbox[] = {
...
#define block_copy(d, s) memcpy(d, s, 16 * sizeof(u8))

void aes_first_addroundkey(u8 state[], const u8 text[], const u8 round_key[]);
void aes_addroundkey(u8 state[], const u8 round_key[]);
void aes_last_addroundkey(u8 text[], const u8 state[], const u8 round_key[]);

void aes_subbytes_shiftrows(u8 state[]);
void aes_subbyte_shiftrows_mixcols(u8 state[]);

void aes_isubbytes_ishiftrows(u8 state[]);
void aes_isubbyte_ishiftrows_imixcols(u8 state[]);

void aes_keyexp(const u8 key[], u8 key_sched[])
{
	u8 key_sched_0, key_sched_1, key_sched_2, key_sched_3, tmp;
